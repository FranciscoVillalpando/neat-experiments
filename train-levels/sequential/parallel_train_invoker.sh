...
do
	...
	PREV_GAME=${!GAME_ARRAY[i-1]:0:1}
	PREV_LEVEL=${!GAME_ARRAY[i-1]:1:1}

	echo "GAME: ${GAME} LEVEL: ${LEVEL} " 
	echo "PREV_GAME: ${PREV_GAME} PREV_LEVEL:${PREV_LEVEL} " 

	python3 retro-train.py $run_id ${GAME} ${LEVEL} ./last_gen_genomes/$run_id/${PREV_LEVEL}* >> ./run_logs/$run_id/${LEVEL}_train_${run_id}.txt
done
