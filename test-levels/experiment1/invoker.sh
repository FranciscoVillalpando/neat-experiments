#!/bin/bash

if [ "$1" == "" ]; then
	echo "Need No. runs..."
	exit 1
fi

echo "Starting invoker script..."
echo "Running $1 cycles for each test game"

GAME_0=("SonicTheHedgehog-Genesis" "SpringYardZone.Act1.state")
GAME_1=("SonicTheHedgehog-Genesis" "GreenHillZone.Act2.state")
GAME_2=("SonicTheHedgehog-Genesis" "StarLightZone.Act3.state")
GAME_3=("SonicTheHedgehog-Genesis" "ScrapBrainZone.Act1.state")

GAME_4=("SonicTheHedgehog2-Genesis" "MetropolisZone.Act3.state")
GAME_5=("SonicTheHedgehog2-Genesis" "HillTopZone.Act2.state")
GAME_6=("SonicTheHedgehog2-Genesis" "CasinoNightZone.Act2.state")

GAME_7=("SonicAndKnuckles3-Genesis" "FlyingBatteryZone.Act2.state")
GAME_8=("SonicAndKnuckles3-Genesis" "HydrocityZone.Act1.state")
GAME_9=("SonicAndKnuckles3-Genesis" "AngelIslandZone.Act2.state")

GAME_ARRAY=(
  	GAME_0[@]
  	GAME_1[@]
    GAME_2[@]
	GAME_3[@]
	GAME_4[@]
	GAME_5[@]
	GAME_6[@]
	GAME_7[@]
	GAME_8[@]
	GAME_9[@]
)

COUNT=${#GAME_ARRAY[@]}
for ((i=0; i<$COUNT; i++))
do
	GAME=${!GAME_ARRAY[i]:0:1}
	LEVEL=${!GAME_ARRAY[i]:1:1}
	echo "NAME ${GAME}"
	echo "LEVEL ${LEVEL}"

	for (( c=1; c<=$1; c++ ))
	do  
		echo "===================================="
		echo "Starting run $c "

		echo "Writing output to run$c.txt"
		
		python3 retro-test.py run$c ${GAME} ${LEVEL} >> ${LEVEL}_run${c}.txt

		#./script.sh run$c.txt
	done

done


 
#./script 

echo "Finishing invoker script..."
